...
class Markdown implements MarkdownInterface {
	...
	protected function _doHeaders_callback_atx($matches) {
		...
		/* 헤더 CLASS 설정 */
		if($level == 1) $classAtt = ' class="chapter_title"';
		elseif($level > 1) $classAtt = ' class="chapter_sub_title"';
		/* 헤더 ID 설정 */
		//$headerTitleArray = explode(" " ,$this->runSpanGamut($matches[2]));
		$headerTitleArray = $this->runSpanGamut($matches[2]);
		$headerTitleID = str_replace(".","",str_replace(" ","-",$headerTitleArray));
		$headerTitleID = str_replace("/","",$headerTitleID);
		$headerTitle = ' id="'.$headerTitleID.'" href="#'.$headerTitleID.'" name="#'.$headerTitleID.'"';


		//if(is_numeric($headerTitleNum)) {
			//$headerTitle = ' id="s_'.str_replace(".","",$headerTitleArray).'"';
		//}

		$block = "<h$level$headerTitle$classAtt>".$this->runSpanGamut($matches[2])."</h$level>";
		return "\n" . $this->hashBlock($block) . "\n\n";
	}
	...
	protected function _doLists_callback($matches) {
		...
		{
			$list_type = "div";
			$notice_css = 'notices yellow element normal';

			$list_strArray = array_unique(explode("\n", $list));
			$result = '<br>';
			foreach($list_strArray as $key) {
				preg_match('/\\[([^\\[\\]]+)\\]\\(\\#/', $key, $returnLinkMatche); //[ 와 ] 문자열
				if(!empty($returnLinkMatche))
				{
					preg_match('/\\(([^\\(\\)]+)\\)/', $key, $returnLinkTextMatche);
					$key = str_replace($returnLinkTextMatche[0], '', $key);
					$key = str_replace($returnLinkMatche[1],"<a href='$returnLinkTextMatche[1]'><strong>".$returnLinkMatche[1]."</strong></a>", $key);
				}

				$key = preg_replace("/^[+]/", "", $key);
				preg_match('/\*\*([^\*\*]+)\*\*/', $key,$returnMatche); // ** 와 ** 사이의 문자열
				if(empty($returnMatche[1]))
				{
					$key = preg_replace("/^[+]/", "", $key);
					$result .= $key."<br>";
				}
				...
				{
					$result .= str_replace($returnMatche[0],"<strong>".$returnMatche[1]."</strong>", $key);
					$result .= "<br>";
				}
			}
			$arr_keys = array_keys($list_strArray);
			$last_key = end($arr_keys);
			if($list_strArray[$last_key] !== '')
			{
				$result .= "<br>";
			}
		}
		...
		{
			$list_type = "div";
			$notice_css = 'notices red element normal';
		...
		else $list_type = preg_match("/$marker_ul_re/", $matches[4]) ? "ul" : "ol";
		...
		if ($ol_start > 1 && $list_type == 'ol'){
			...
			if($list_type == 'div')
			{
				$result = $this->hashBlock("<$list_type class='".$notice_css."'>\n" . $result . "</$list_type>");
			}
			else
			{
	...
	protected function _processListItems_callback($matches) {
		...
		return "<li class='element normal'>" . $item . "</li>\n";
	}
